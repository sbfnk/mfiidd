...
my_logPosterior <- function(fitmodel, theta, init.state, data) {
    ...
    log.prior <- # INSERT HERE: calculate the fitmodel log-prior
        # of `theta` using `fitmodel$logPrior`, and assign
        # to variable `log.prior`

    log.likelihood <- # INSERT HERE: calculate the log-likelihood of `theta`
        # and `init.state` with respect to the data using `trajLogLike`
...
If you run into any problems, have a look at our [solution](posterior_example_solution.md). Otherwise, [return to the practical](introduction.md#calculate-the-posterior).
