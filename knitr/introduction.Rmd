...
The function `genObsPoint` generates a single observation from a point in a model trajectory. In that sense, it can be seen as the inverse of `pointLogLike`. Whereas `poitnlogLike` evaluates the log-likelihood at a data point with respect to the model, `genObsPoint` takes a (randomly sampled) data point from the model.
...
You can assess a model run against data using the `plotFit` function, which generates an observation trajectory from a model, parameters and initial conditions using `genObsTraj` (see above) and plots it (lines) against the data (points). You can also simulate multiple replicates (using the `n.replicates` argument) or plot the deterministic model trajectories (using `all.vars = TRUE`). See `?plotFit` for all available options.
...
Try different distributions for the prior and likelihood distributions. Do they change the shape of the posterior distribution?
