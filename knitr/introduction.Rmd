...
This is contains several epidemic data sets that have been created using this same SIR model, with an infectious period of 2, in a population of 1000, with 1 initial infected and the remaining population susceptible. You can look at a first data set using
...
Now we are ready to do parameter estimation by exploring the posterior at different values of the parameter. As stated above, the infectious period of the `sir.epi1` data set was 2. Evaluate the posterior at different values of $R_0$ using the `my_posterior` function you wrote above (or the `our_posterior` function provided by clicking through to the solution). In which range of $R_0$ is the posterior maximised?
...
SIR$pointLogLike <- function(data.point, model.point, theta) {
        ## the prevalence is observed through a normal process
	return(dnorm(x=data.point[["obs"]], lambda=mean.point[["I"]], log=TRUE))
}
```
...
If you knew that on average only 10% of cases were reported, how would you change the point likelihood. You can test this with a second data set, `epi2`, which you can load with
