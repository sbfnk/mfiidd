...
```{r setup, echo=FALSE}
opts_chunk$set(cache=TRUE, cache.path="cache/SEITL_model_example/", fig.retina=1 ,fig.path="figure/SEITL_model_example/", fig.cap='', fig.height=10, fig.width=10, dpi=100, fig.align="center", message=FALSE, tidy=TRUE, warning=FALSE)
```
...
Although our example refers to the SEITL model, the same commands work for the SEIT2L model (i.e. `example(SEIT2L_deter)` instead of `example(SEITL_deter)`).
...
You can now go back to the [practical](play_with_seitl.md#run-a-mcmc) and try to run a MCMC with those settings.

# Run a MCMC

If you didn't manage to run a MCMC, or it took too long to obtain a few thousand iterations, you can load our short-run as follows:
...
```
You can now go back to the [practical](play_with_seitl.md#short-run-analysis) and analyse this trace.
...
```
The thinned trace shows a smoother distribution despite having less samples than the unthinned one. This because the local "bumps" of the unthinned distribution are caused autocorrelated samples.

You can now go back to the [practical](play_with_seitl.md#long-run-analysis) and perform a similar analysis for a long-run MCMC.

# Long run analysis

Here is an example of analysis for our long run ($10^5$ iterations)
...
xyplot(trace)
```
Note that the acceptance rate and the ESS are computed for the combined chain whereas the traces are plotted for each chains.
Also, given the very high ESS we can reasonably choose a burning visually, say 5000 iterations.
...
```{r long-run-thin}
# let's keep 1 iteration every 40
...
# Note that plotPosteriorDensity can't take a list of mcmc.list.
# Instead, we pass the unthinned and thinned traces of the chain that started at theta1
plotPosteriorDensity(list(unthinned=trace.burn[[1]],thinned=trace.burn.thin[[1]]))
```
...
You can now go back to the [practical](play_with_seitl.md#correlations) and look at the posterior correlations between the parameters.
...
You can now go back to the [practical](play_with_seitl.md#informative-priors) and run a MCMC with this informative prior.
...
You can now go back to the [practical](play_with_seitl.md#model-selection) for the final exercise of the session.
...
x <- ldply(trace.info.burn.thin)

theta_bar <- colMeans(x[SEIT2L_deter$theta.names])

init.state <- c(S = 279, E = 0, I = 2, T = 3, L = 0, Inc = 0)
init.state <- c(S = 279, E = 0, I = 2, T1 = 3, T2 = 0 , L = 0, Inc = 0)

ll_theta_bar <- trajLogLike(SEIT2L_deter,theta_bar, init.state, data=FluTdC1971)

D_theta_bar <- -2*ll_theta_bar

p_D <- 1/2*var(-2*x$log.likelihood)
