...
[Return](ABC.md#run-mcmc) to the ABC session.
...
# deterministic fit
init.theta <- colMeans(trace[SEITL_stoch$theta.names])
...
Now we can define a wrapper function for the ABC posterior. Let's try $\epsilon = 0.25$
...
my_ABCLogPosterior_tdc_det <- function(theta) {
    ...
    # acceptance window of 0.25
    log.posterior <-
        ABCLogPosterior(epsilon = 0.25,
...
```{r eval = F}
deter.theta <- summary.mcmc$statistics[, 1][SEITL_deter$theta.names]
...
[Return](ABC.md#comparison-with-exact-posterior) to the MCMC session.
