...
```{r setup, echo=FALSE}
opts_chunk$set(cache=TRUE, cache.path="cache/SEITL_play_example/", fig.retina=2 ,fig.path="figure/SEITL_play_example/", fig.cap='', fig.height=4, fig.width=5, dpi=100, fig.align="center", message=FALSE, tidy=TRUE, warning=FALSE)
```
...
Now, you can now go back to the [practical](play_with_seitl.md#deterministic-vs-stochastic-simulations) and check whether these estimates provides a good fit to the data.
...
Now, you can now go back to the [practical](play_with_seitl.md#stochastic-simulations) and check whether the stochastic model provides a better fit to the data.
...
Now, you can now go back to the [practical](play_with_seitl.md#exponential-vs-erlang-distributions) and check how we can make this model a bit more realistic.
...
You've already finished? why not [going further?](play_with_seitl.md#going-further)
