...
There is one small extra step we have to do before we can sample from `dnorm`. Remember that the we have set up the Metropolis-Hastings sampler above to expect the target distribution to return the logarithm of the probability density, whereas `dnorm`, by default, returns the (un-logged) probability density directly.
...
```{r}
my_logPosterior_R0(c(R0 = 3, D.inf = 2))
```
