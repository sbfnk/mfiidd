...
```{r setup, echo=FALSE}
opts_chunk$set(cache=TRUE, cache.path="cache/mcmc/", fig.retina=1 ,fig.path="figure/mcmc/", fig.cap='', dpi=100, fig.align="center", message=FALSE, tidy=TRUE, warning=FALSE)
```
...
```
 ...
 ```{r plot_trace, caption = "", fig.width = 10}
 plot(trace, type = "l")
 ```
...
We can now generate samples from `my_logPosterior_epi1` using `my_mcmcMH`. Can you work out the command to do this? If have any problems with this, have a look at our [solution](generate_samples.md).
