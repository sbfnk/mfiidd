...
Here, we will look at a library called *LibBi* (Library for Bayesian inference). It can be used from R via the `rbi` package.
...
devtools::install_github('sbfnk/rbi.helpers')
```
...
```r
SEITL_deter <- bi_model(lines=readLines("https://sbfnk.github.io/mfiidd/bi/SEITL_deter.bi"))
```
...
Now, we can use this data set and the model to run pMCMC. First, we generate 3000 samples where we use the prior distribution as proposal distribution, to find a good starting point for the MCMC chain:
...
for (i in 1:max_erlang) {
  bi[[i]] <- sample(prep, model=adapted$model %>% fix(k_erlang=i), 
...
It looks like the best is 4, although only slightly better than 1. You can plot the fit plotting filtered trajectories as generated by the pMCMC algorithgg
