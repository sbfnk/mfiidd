...
Here you could use the function `my_rmcmcMH` that you have already coded but it might result in poor acceptance rates since the models have 6 parameters to be estimated. Because of this, using the adaptive MCMC might be a better choice.

__Take 5 minutes__ to have a look at the adaptive MCMC implemented in the function `mcmcMH`. As you can see, this function takes similar arguments as your function `my_rmcmcMH` (`target`, `init.theta`, `proposal.sd` and `n.iterations`) plus several new ones that control the adaptive part of the algorithm and that you can learn about via the help page of `mcmcMH`. Note also that this function returns a list that contains, in addition to the trace, the acceptance rate and the empirical covariance matrix of the posterior. The latter will be useful for improving the covariance matrix of the Gaussian proposal in the next session, when we will fit the stochastic model.
...
However, as you will see at the end of this session, in order to be able to compare different models we also need to track the log-likelihood of the sampled `theta`s. Although this could be done by running `dObsTraj` on each returned `theta`, you might remember that the log-likelihood is actually computed in `my_dLogPosterior` so we could just use it. This is exactly what the function `logPosterior` does for you.

__Take 5 minutes__ to look at the code of `logPosterior`. As you can see, this function takes similar arguments as your function `my_dLogPosterior` (`fitmodel`, `theta`, `init.state` and `data`) plus one called `margLogLike`, which is a function to compute the log-likelihood of `theta`. By default, it is set to `dObsTraj` so you don't really need to bother with this argument in this session. However, it will be useful in the next session as we will need a different function than `dObsTraj` to compute the log-likelihood for a stochastic model. Note also that this function returns a list of two elements:
...
Now it's time to use what you've learned in the previous session to analyse your MCMC outputs using the `coda` package. You didn't manage to run `mcmcMH`? Just use the results from our [example](example_mcmc_SEITL.html#run-mcmc).
...
Each chain took 5 hours on a scientific computing cluster (thanks [Bernard](external_ref/bernard-cazelles.jpg)) and can be loaded as follows:
...
my_postPredCheck <- function(trace, n.samples, fitmodel, init.state, data) {
    ...
    ## start for() loop over sampled numbers
        ...
        # use rObsTraj to generate observation trajectory using row
        # from trace
