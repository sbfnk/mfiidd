...
set.seed(1234)
```
[Lecture slides](slides/smc_2018.pdf)
...
Note that the function `dLogPosterior` doesn't have a `n.particles` argument so you might wonder how to specify it to `my_particleFilter`? Have a look at the documentation of `dLogPosterior`. You should notice the `...` argument, which allows you to pass any extra argument to the function `margLogLike`.
