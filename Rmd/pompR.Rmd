...
```
```{r include = FALSE}
source("pomp/seit4lPompR.r", echo = TRUE)
```
...
seitlTraj <- trajectory(seitlPompR,
...
```{r}
pf <- pfilter(seitlPompR, params = c(theta, seitlInitState), Np = 100)
logLik(pf)
```
...
```{r results = "hide"}
seitlTm <- traj_objfun(seitlPompR,
  params = c(theta, seitlInitState),
  est = names(theta)
)
...
seitlMf <- mif2(
  seitlPompR, params = coef(seitlTm), Nmif = 50, Np = 1000, 
...
seitlPmcmc <- pmcmc(
  seitlPompR, params = coef(seitlMf), Nmcmc = 5000, Np = 128, 
...
```{r eval = FALSE, fig.height = 12, fig.width = 10}
library("coda")
# acceptance rate
