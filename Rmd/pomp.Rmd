...
The `pomp` objects `SEITL_pomp` and `SEIT2L_pomp` now contain everything needed for model fitting and inference. Let's, again, guess some parameters and initial values, noting the *pomp* syntax, whereby initial states are named as the named variable with `.0` appended,
...
and try the following operations (with SEITL or SEIT2L):
...
This takes the deterministic skeleton as specified in the pomp object, and ignores all process or measurement stochasticity.
