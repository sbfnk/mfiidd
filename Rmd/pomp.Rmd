...
```{r load-seit-pomp, include = FALSE}
source(here::here("scripts", "snippets", "seitl-pomp.r"))
source(here::here("scripts", "snippets", "seitl-pomp-r.r"))
```
...
```{r}
theta <-
  c(R_0 = 2, D_lat = 2, D_inf = 2, alpha = 0.9, D_imm = 13, rho = 0.85)
SEITL.initState <-
...
SEITL.pm <- pmcmc(SEITL.tm, Nmcmc = 5000, Np = 200, proposal = mvnRw.adaptive(rw.sd = prop.sd, scale.start = 100, shape.start = 200), max.fail = Inf)
