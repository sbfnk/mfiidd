...
```{r results = "hide"}
source("https://sbfnk.github.io/mfiidd/pomp/SEITL_pomp.r", echo = TRUE)
```
...
__Note__: For the examples presented here, we are providing an alternative version that is coded entirely in *R*. To look at this, simply add "\_r" to the file names (before the `.r` extension). For example, to see the *R* version of the `SEITL` model in *pomp*, type 
```{r results = "hide"}
source("https://sbfnk.github.io/mfiidd/pomp/SEITL_pomp.r", echo = TRUE)
```
...
```{r results = "hide"}
source("https://sbfnk.github.io/mfiidd/pomp/SEIT4L_pomp.r", echo = TRUE)
```
...
SEITL.initState <- c(S.0 = 250, E.0 = 0, I.0 = 4, T.0 = 0, L.0 = 30, Inc.0 = 0)
SEIT4L.initState <- c(
...
SEITL.traj <- trajectory(SEITL_pomp,
  params = c(theta, SEITL.initState),
  as.data.frame = TRUE
)
plotTraj(SEITL.traj)
```
...
```{r}
pf <- pfilter(SEITL_pomp, params = c(theta, SEITL.initState), Np = 100)
logLik(pf)
```
...
```{r results = "hide"}
SEITL.tm <- traj.match(SEITL_pomp,
  start = c(theta, SEITL.initState),
  est = names(theta)
)
```
```{r}
...
SEITL.mf <- mif(
  SEITL.tm, Nmif = 50, Np = 1000, cooling.fraction.50 = 0.01, rw.sd = prop.sd
)
SEITL.mf.sim <- simulate(
  SEITL.mf, nsim = 10, as.data.frame = TRUE, include.data = TRUE
  )
...
SEITL.pm <- pmcmc(
  SEITL.tm, Nmcmc = 5000, Np = 200, 
  proposal = mvnRw.adaptive(
    rw.sd = prop.sd, scale.start = 100, shape.start = 200
  ), max.fail = Inf
)
