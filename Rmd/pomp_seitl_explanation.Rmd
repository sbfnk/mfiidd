...
Instead of calculating the rates of transition as in the deterministic skeleton above, we now draw random numbers from each compartment to get the number of individuals who transition from that compartment to another one. These numbers are stored in the array `N`, and the numbers in each compartment adjusted in the following segment of code:
...
This constructs the pomp object from the C functions defined above. The individual options are:
