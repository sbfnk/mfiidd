...
Of course, you might get a different number here if you played with the parameters. You will later need the `trajLogLike` function to calculate the posterior density as a function of the parameters.
...
To generate a whole trajectory with simulated observation, we have implemented a function called `genObsTraj`, which can be seen as the inverse of `trajLogLike`. The `genObsTraj` function uses the `simulate` function (see above, where you looked at `SIR$simulate`) to simulate the model, and then apply the `rPointObs` function at every time point to generate the observations. It returns the simulated trajectory, with an added column for the observations.
