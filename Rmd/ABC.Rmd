...
```{r setup, echo=FALSE}
opts_chunk$set(cache=TRUE, fig.path='figure/ABC/', cache.path='cache/ABC/', fig.cap='', fig.align="center",tidy=FALSE)
```
...
You can now plug this into the `mcmcMH` (or `my_mcmcMH`, if you would like to use your own) function. Can you work out the command to run ABC-MCMC? If not, have a look at the [solution](abc_solution.html#abc-mcmc-on-the-stochastic-model).
...
You can now explore this using the `mcmcMH` function, and the same command as before. Compare this to MCMC using the `SEITL2_deter` function directly (in a posterior wrapper function, as you did [here](play_with_seitl.html#run-mcmc)). Compare the two using `coda` plot functions. How good is your the ABC estimate using different summary statistics?

If you have any trouble running the MCMC, have a look at our [solution](abc_solution.html#comparison-with-exact-posterior).
...
<div>
# Navigate
Top: [Index](index.html) Previous: [Particle MCMC](pmcmc.html)
<div>
