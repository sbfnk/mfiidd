...
```{r setup, echo=FALSE}
knitr::opts_chunk$set(cache=TRUE, cache.path="cache/example_mcmc_SEITL/", fig.retina=2 ,fig.path="figure/example_mcmc_SEITL/", fig.cap='', fig.align="center", message=FALSE, tidy=TRUE, warning=FALSE)
```
...
my_posteriorTdC <- function(theta){
	...
	return(logPosterior(fitmodel= my_fitmodel, theta=theta, init.state= my_init.state, data=FluTdC1971, margLogLike = dObsTraj))
...
You can now go back to the [practical](mcmc_and_model_comparison.html#run-mcmc) and try to run MCMC with those settings.
...
init.state <- c(S = 279, E = 0, I = 2, T = 3, L = 0, Inc = 0)
log.like.theta.bar <- dObsTraj(SEITL_deter,theta.bar, init.state, data=FluTdC1971)
