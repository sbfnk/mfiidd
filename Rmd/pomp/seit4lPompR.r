seit4lSkelR <- function(S, E, I, T, L, Inc, R0, D_inf, D_lat, D_imm, alpha,
                             rho, ...) {
  beta <- R0 / D_inf
  epsilon <- 1 / D_lat
  nu <- 1 / D_inf
  ...
  c(
    S = DS, E = DE, I = DI, T1 = DT1, T2 = DT2, T3 = DT3, T4 = DT4, DL = DL,
    DInc = DInc
  )
}

seit4lSimR <- function(S, E, I, T, L, Inc, R0, D_inf, D_lat, D_imm, alpha, rho,
                       delta.t, ...) {
  beta <- R0 / D_inf
  epsilon <- 1 / D_lat
  nu <- 1 / D_inf
  ...
  c(S = S, E = E, I = I, T1 = T1, T2 = T2, T3 = T3, T4 = T4, L = L, Inc = Inc)
}

## construct pomp object
seit4lPompR <- pomp(
  data = fluTdc1971[, c("time", "obs")],
  skeleton = vectorfield(seit4lSkelR),
  rprocess = discrete_time(step.fun = seit4lSimR, delta.t = 0.1),
