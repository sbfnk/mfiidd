SEIT4L.skel.r <- function(x, t, params, ...) {
...
SEIT4L.sim.r <- function(x, t, params, delta.t, ...)
{
    with(as.list(c(x, params)),
    {
        beta = R0 / D_inf;
...
SEIT4L_pomp_r <- pomp(data = fluTdc1971[, c("time", "obs")],
                      skeleton = vectorfield(SEIT4L.skel.r),
                      rprocess = euler.sim(step.fun = SEIT4L.sim.r,
                                           delta.t = 0.1),
