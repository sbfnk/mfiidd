...
for (testR0 in seq(1.6, 1.9, length.out = 25))
{
  ...
  lp <- my_dLogPosterior(SIR, theta, initState, epi1)
...
iter <- 1000
trace <- my_mcmcMh(
  target = dnormLog, initTheta = startingValue,
...
```{r write_logpost}
my_dLogPosteriorR0Epi1 <- function(R_0) {
  return(my_dLogPosterior(
    fitmodel = SIR,
    theta = c(R_0 = R_0, D_inf = 2),
...
```{r eval_logpost}
my_dLogPosterior_R0Epi1(R_0 = 3)
```

You should get the same number unless you changed the `SIR$dPointObs` function.

__Take 10 minutes to generate samples from `my_dLogPosterior_R0Epi1` using `my_mcmcMh`. Can you work out the command to do this?__ If you have any problems with this, have a look at our [solution](generate_samples.html).
