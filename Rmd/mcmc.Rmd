...
theta <- c(R0 = 3, D_inf = 2) # parameter vector
init.state <- c(S = 999, I = 1, R = 0) # initial conditions
grid <- NULL # set aside to hold the grid approximation

# Loop through each value of R0 we want to test.
for (my_R0 in seq(1.6, 1.9, length.out = 25))
{
    # Set R0 in theta accordingly
    theta[["R0"]] <- my_R0
    
    # Evaluate the log posterior associated with this R0
    lp <- my_dLogPosterior(SIR, theta, init.state, epi1)
    
    # Save this iteration, using rbind to add another row to the grid data frame
    grid <- rbind(grid, data.frame(R0 = my_R0, lp = lp))
}
```

Run the code above, first making sure that `my_dLogPosterior`, `SIR`, and `epi1` are still available from the previous session.
...
grid[max_row, ] # log posterior probability and R0 at that row
```
...
```{r grid_plot}
plot(x = grid[["R0"]], y = exp(grid[["lp"]]), type = "p", xlab = "R0", ylab = "posterior")
```
...
weighted.mean(grid[["R0"]], w = exp(grid[["lp"]]))
```
...
for (i in 1:1000)
{
    # Select a random value for R0
    my_R0 <- runif(1, 1.7, 1.8)
    theta[["R0"]] <- my_R0
    ...
    if (runif(1) < exp(lp - max_lp)) {
        samp <- rbind(samp, data.frame(R0 = my_R0, lp = lp))
    }
}
```
