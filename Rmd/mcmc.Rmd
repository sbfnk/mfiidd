...
```{r load, echo=FALSE, cache=FALSE, results="hide",message=FALSE}
library(fitR)
data(SIR)
data(SIR_reporting)
```
...
This examples looks reassuringly similar to the normal distribution centred around 0. Try different values for `init.theta` and `proposal.sd`. How do these affect the plots of the trace and the histogram? For more information can also look at our [discussion](mcmc_discussion.md) of this.
