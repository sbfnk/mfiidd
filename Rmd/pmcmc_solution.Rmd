...
for(n.particles in test.n.particles){
	...
	for(i in 1:n.replicates){
		# one Monte-Carlo estimate of the log-likelihood
            sample.log.like[i] <- my_particleFilter(SEIT2L_stoch, theta,
...
init.state <- c(S = 279, E = 0, I = 2, T1 = 3, T2 =0, L = 0, Inc = 0)
log.like.theta.bar <- my_particleFilter(SEIT2L_stoch, theta.bar, init.state,
...
plotFit(SEIT2L_stoch, theta.bar, init.state, data = FluTdC1971, n.replicates = 1000)

plotFit(SEIT2L_deter, theta.bar.deter, init.state,data = FluTdC1971, n.replicates = 1000)
