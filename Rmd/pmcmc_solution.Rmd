...
```{r calibration-SMC, file = "../R/calibrateSmc.r", eval = FALSE, tidy = FALSE}
```
...
dfPlot <- melt(calibrateSmc,id.vars="nParticles")

dfPlot <- mutate(dfPlot, variable=revalue(variable,c(time_10000iter_day="time 10000 iter (in days)", prop_depleted="prop. of samples with particle depletion")))

p <- ggplot(dfPlot, aes(x=nParticles,y=value))+facet_wrap(~variable,scales="free_y")
...
covmat <- mcmc_SEITL_infoPrior_theta1$covmatEmpirical
...
```{r analyse-n50, collapse=TRUE, fig.height=7, fig.width=10}
# load traces
...
```{r analyse-n400, collapse=TRUE, fig.height=7, fig.width=10}
# load traces
data(pmcmc_SEIT4L_infoPrior_n400)

# combine into a `mcmc.list` object
library('coda')
trace <- mcmc.list(lapply(pmcmc_SEIT4L_infoPrior_n400, function(chain) { mcmc(chain$trace) }))
