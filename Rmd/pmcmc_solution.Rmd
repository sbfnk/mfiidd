...
for(n.particles in test.n.particles){
        ...
        cat("Testing ", n.particles, " particles\n")
...
The burn-in period looks relatively short, this is because we started the chains with a "good" `init.theta`. That said, since we have relatively short runs it might be useful to carefully choose the burn-in period with `plotESSBurn`.
...
log.like.theta.bar.deter <- dTrajObs(SEIT4L_deter, theta.bar, init.state,data = FluTdC1971, log=TRUE)
