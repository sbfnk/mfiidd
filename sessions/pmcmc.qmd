...
Gillespie step (`src/seit4l_gillespie.jl`):

```{.julia include="../src/seit4l_gillespie.jl"}
```

SSMProblems interface (`src/seit4l_ssm_interface.jl`):

```{.julia include="../src/seit4l_ssm_interface.jl"}
```

Bootstrap filter runner (`src/seit4l_runner.jl`):
...
All these components were loaded via `using MFIIDD` in the setup above (defined in `src/seit4l_gillespie.jl`, `src/seit4l_ssm_interface.jl`, and `src/seit4l_runner.jl`). Now we'll use them for full Bayesian inference.
...
## Code explainer
- **`StateSpaceModel(...)`** - assembles the state-space model from initial distribution, dynamics, and observation components defined in `src/seit4l_ssm_interface.jl`.
