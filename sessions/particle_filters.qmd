...
```{julia}
# Propagate 20 particles from day 4 to day 5
n_particles = 20

# Start particles from a realistic state at day 4
# (run forward from initial state to get there)
particles_day4 = []
for _ in 1:n_particles
    state = Float64[init_state[:S], init_state[:E], init_state[:I],
                    init_state[:T1], init_state[:T2], init_state[:T3],
                    init_state[:T4], init_state[:L]]
    for day in 1:4
        state, _ = gillespie_step(Random.default_rng(), state, θ_example)
    end
    push!(particles_day4, copy(state))
end

# Now propagate one more day and record the incidence
day_5_predictions = Float64[]
for i in 1:n_particles
    _, inc = gillespie_step(Random.default_rng(), particles_day4[i], θ_example)
    push!(day_5_predictions, inc)
end

# Plot: show propagation from day 4 to day 5
p = plot(title="Particle Filter: Propagation Step",
         xlabel="Time (days)", ylabel="Daily incidence")

# Draw lines from previous observation to predicted incidence
day_4_observed = flu_tdc.obs[5]  # Observation at day 4
for i in 1:n_particles
    plot!(p, [4, 5], [day_4_observed, day_5_predictions[i]],
          alpha=0.5, color=:steelblue, linewidth=1.5, label="")
end

# Previous observation (gray) - where particles started
scatter!(p, [4], [day_4_observed],
         label="Previous observation", markersize=6, color=:gray)

# Current observation (red) - used for weighting
day_5_observed = flu_tdc.obs[6]
scatter!(p, [5], [day_5_observed],
         label="Current observation", markersize=8, color=:red)

plot!(p, legend=:topright)
```

Each blue line shows one particle's predicted incidence for day 5. The particles spread out because each follows different random events. The current observation (red) will be used to weight the particles in the next step.
...
```{julia}
# Weight particles based on how well they match the observation
# (day_5_predictions and day_5_observed defined in previous code block)
