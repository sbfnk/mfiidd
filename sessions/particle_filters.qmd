...
[Lecture slides](slides/particle_filters.qmd)
...
Look at the variance plot (middle). We want the SD to be in the range 1-3, and the curve drops through that window steeply rather than settling inside it, so read off the smallest count that brings the SD comfortably below 3.
...
- **32-64 particles**: SD falling steeply, and swinging a good deal from run to run
- **128 particles**: SD comfortably below 1
- **256-512 particles**: SD of around 0.5 or less, diminishing returns for the extra computation

Your numbers will differ due to stochastic variability, and at the low counts they differ a lot: the spread is itself estimated from a handful of very noisy runs. Read the shape of the curve rather than any single value, and leave yourself headroom above the count that first looks acceptable. For the PMCMC session, we'll use around 128-256 particles based on this calibration.
:::
...
are. A count calibrated at the posterior mode will be too few out in the tails,
which is precisely where a chain gets stuck and stays stuck. Calibrating at a
reasonable parameter set and then taking some headroom is cheaper than diagnosing
a chain that will not move.
:::
