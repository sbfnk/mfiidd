...
# Past observations (already processed) in gray
scatter!(p, flu_tdc.time[1:10], flu_tdc.obs[1:10],
         label="Past observations", markersize=4, color=:gray, alpha=0.5)

# Current observation (used for weighting) in red
scatter!(p, [flu_tdc.time[11]], [flu_tdc.obs[11]],
         label="Current observation", markersize=6, color=:red)

plot!(p, legend=:topright)
```

Each blue line is one "particle" - a possible trajectory. The particles spread out because each follows different random events. The current observation (red) will be used to weight the particles; past observations (gray) have already been processed.
