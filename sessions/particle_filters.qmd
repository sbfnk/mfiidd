...
## Coming from R?
Particle filters weren't covered in the R-based course. If this is your first encounter with them, that's expected — they're a new topic rather than a Julia translation of something you've seen before. In R, the [`pomp`](https://kingaa.github.io/pomp/) package and the [`odin`](https://mrc-ide.github.io/odin/)/[`dust`](https://mrc-ide.github.io/dust/)/[`mcstate`](https://mrc-ide.github.io/mcstate/) ecosystem provide particle filtering capabilities.
:::
...
::: {.callout-tip}
## Workflow note
In your own work, you should run [prior and posterior predictive checks](model_checking.qmd) when fitting models. We skip these here to focus on the particle filter mechanics.
:::
...
Here is the full interface implementation (defined in [`src/seit4l_ssm_interface.jl`](https://github.com/sbfnk/mfiidd/blob/main/src/seit4l_ssm_interface.jl)):
...
This block uses a few Julia features worth unpacking:

- `struct SEIT4LDynamics <: SSMProblems.LatentDynamics` defines a new *composite type* — essentially a container that holds some data (here, the parameter dictionary `θ`). The `<:` means "is a subtype of". `SSMProblems.LatentDynamics` is an *abstract type*: a type with no fields of its own, used purely to mark "this thing plays the role of a latent-dynamics model". By declaring our struct a subtype, we tell `SSMProblems.jl` that our type fills that role. The same pattern applies to `SEIT4LInitial <: SSMProblems.StatePrior` and `PoissonObservation <: SSMProblems.ObservationProcess`.

- `function SSMProblems.simulate(rng, dyn::SEIT4LDynamics, step, prev_state; kwargs...)` adds a new *method* to the existing function `SSMProblems.simulate`. The `dyn::SEIT4LDynamics` annotation tells Julia: "use this method when the second argument is of our type". This is [multiple dispatch](https://docs.julialang.org/en/v1/manual/methods/): one function name, many methods, chosen by argument types. `SSMProblems.jl` knows to call `simulate(rng, dynamics_object, ...)` and will pick up our method for `SEIT4LDynamics`. We implement `simulate` for the latent dynamics (how the state evolves one step) and for the initial state (how to draw the first state). For the observation model we instead implement `distribution`, which returns a `Distributions.jl` distribution describing how observations arise from a given state.

- `rng::AbstractRNG` is Julia's abstract random number generator type. Any concrete RNG (the default one, a seeded Mersenne twister, etc.) is a subtype. Threading the `rng` through every simulation step is what lets the particle filter control randomness — set the seed once and you get reproducible particles.

- `kwargs...` at the end of a function signature means "accept any additional keyword arguments and collect them into a named tuple called `kwargs`". We don't use them here, but `SSMProblems.jl` may pass extra keyword arguments (e.g. time steps, metadata) depending on the filter being used, so our methods need to accept them without complaining.

- `vcat(prior.init_state, 0.0)` concatenates vertically — it takes the 8-element state vector and appends `0.0` to make a 9-element vector. We use the 9th slot to track daily incidence, because the observation process needs to see it. `collect(prev_state[1:8])` grabs the first 8 elements (the compartments) and copies them into a plain `Vector` — some inputs arrive as views or other array-like types, and `gillespie_step` wants a concrete vector.

**Coming from R?** Julia structs with `<:` play a similar role to S4 classes, where `setClass("Foo", contains = "Bar")` declares `Foo` as a subclass of `Bar`. Multiple dispatch is like S4's generic functions: you register methods for specific type combinations, and the runtime picks the right one.
:::
...
::: {.callout-note collapse="true"}
## Typical results (if you can't run the code)
