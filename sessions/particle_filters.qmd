...
Your numbers will differ, and at the low counts they differ a lot: the spread is itself an estimate, which is why the cell above averages over 200 runs rather than a handful. With only 30 runs the value at 64 particles wanders between roughly 1 and 5 depending on which draws you happen to get.

For the PMCMC session we use 128-256 particles even though 64 clears the target here. The exercise below asks why you would go above the minimum.
:::
...
are. A count calibrated at one parameter set will be too few elsewhere, and
elsewhere is precisely where a chain gets stuck and stays stuck. The [particle
MCMC session](pmcmc.qmd) measures this against the fitted posterior and finds the
minimum from this plot failing on a large share of it. Calibrating at a
