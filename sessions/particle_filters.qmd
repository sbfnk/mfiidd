...
This is the implementation the filter below actually calls, shown from the package source rather than repeated by hand, so that the two cannot drift apart:
...
# For weighting step later. The particles were stepped five times from day 0, so
# `trajectories[i][6]` is each particle's incidence on day 5, and the observation
# it should be weighted against is the one recorded on day 5.
