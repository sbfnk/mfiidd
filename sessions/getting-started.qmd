...
%%{init: {'theme': 'neutral'}}%%
flowchart TB
    subgraph core["CORE SESSIONS"]
        direction LR
        A[Getting Started] --> B[Introduction]
        B --> C[MCMC]
        C --> D[Diagnostics]
        D --> E[Model Checking]
    end

    subgraph stoch["STOCHASTIC MODELS"]
        direction LR
        F[SEITL Interlude] --> G[Particle Filters]
        G --> H[Particle MCMC]
    end

    subgraph adv["ADVANCED SESSIONS"]
        direction LR
        I[Observation Models]
        J[Variational Inference]
        K[Universal DEs]
    end
...
| Tier | Sessions | Time |
|------|----------|------|
| **Core** | Getting Started → Introduction → MCMC → Diagnostics → Model Checking | 10-12 hrs |
| **Stochastic** | SEITL Interlude → Particle Filters → Particle MCMC | 5-6 hrs |
| **Advanced** | Observation Models, Variational Inference, Universal DEs (pick any) | 2-3 hrs each |

> **Stochastic sessions** involve stochastic simulation - expect different results each run. This is normal.
> **Advanced sessions** are independent. VI requires only MCMC (can skip stochastic). UDEs requires only Introduction.
