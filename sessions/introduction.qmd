...
```{julia}
# Load epidemic data shipped with the MFIIDD package
epi1 = load_epi1()
...
::: {.callout-tip collapse="true"}
## How does `load_epi1()` know where the data is?
`load_epi1()` is defined in the `MFIIDD` course package (`using MFIIDD` in the setup). It uses `pkgdir(MFIIDD)` to find the package directory and reads `data/epi1_synthetic.csv` from there, so file paths work the same on any machine without you having to manage them.
:::
...
::: {.callout-tip collapse="true"}
## Going further: the LogDensityProblems interface
This log-density function — prior + likelihood as a function of parameters — is the fundamental building block of Bayesian inference. Turing.jl wraps this pattern with convenient syntax, but the same function works with any sampler in the Julia ecosystem via the [LogDensityProblems.jl](https://github.com/tpapp/LogDensityProblems.jl) standard interface. See the [reference page](../reference/logdensity_interface.qmd) for a worked example using DynamicHMC.jl.
:::
