...
We will build intuition by manually calculating priors, likelihoods, and posteriors using an SIR epidemic model, before turning to algorithms that explore the posterior systematically in the next session.
...
::: {.callout-warning}
Uniform distributions are a common first choice, but the hard boundary at the edges (in our case, zero probability for $R_0 = 0.99$ but $1/19$ of a unit of density for $R_0 = 1.01$) can cause numerical problems for many samplers.
...
If those trajectories look implausible, our priors are placing weight on parameter values we don't actually believe are reasonable.
...
(One imperfection remains: a Normal for $R_0$ technically places tiny probability on negative values.
In practice you'd choose a `LogNormal` or a `truncated` Normal to guarantee positivity — we'll pick this up in later sessions.)
...
We start by loading some observed epidemic data (which we simulated from the model, but in principle this could be any data) representing the number of infectious individuals at different time points:
