...
**Take 5 minutes**: Think about when these assumptions might be violated:
...
But we don't know the true parameters - we have a distribution of plausible values. Let's see what happens when we also vary the parameters:
...
**Take 5 minutes**: Does the real data look consistent with our simulated datasets? This is *posterior predictive checking* - a key model validation technique that we'll explore in depth in the [model checking](model_checking.qmd) session.
...
We began by exploring **priors** - our beliefs about parameters before seeing data. Through *prior predictive simulation*, we saw how different prior choices lead to different predicted epidemic trajectories, helping us validate whether our assumptions are reasonable.

We then introduced observed data and learned how the **likelihood** quantifies the probability of seeing that data given specific parameter values. Our Poisson observation model assumes each data point is an independent draw around the model's predicted prevalence - assumptions worth questioning for real applications.

Combining prior and likelihood via **Bayes' theorem** gives us the **posterior** - our updated beliefs after seeing data. We explored this manually through grid search, finding the best-fitting parameters and visualising the posterior surface. Finally, we used *posterior predictive checking* to validate that our fitted model generates data consistent with observations.

The key limitation we encountered is scalability: grid search becomes computationally infeasible as the number of parameters grows. This motivates the algorithmic approaches we'll learn next.
