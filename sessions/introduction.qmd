...
::: {.callout-note collapse="true"}
## Coming from R?
In R, `deSolve::ode(y, times, func, parms)` defines and solves the ODE in one call. Julia splits this into two steps: `ODEProblem()` defines the problem, then `solve()` runs it. This separation lets you reuse the same problem with different solvers or settings.
:::
...
::: {.callout-note collapse="true"}
## Coming from R?
`plot!()` in Julia adds to an existing plot, similar to `lines()` or `points()` in base R, or adding layers with `+ geom_*()` in ggplot2. The plain `plot()` (without `!`) creates a new plot, like `plot()` in base R or `ggplot()`.
:::
...
::: {.callout-note collapse="true"}
## Coming from R?
Watch out for the Gamma parameterisation: Julia's `Gamma(shape, scale)` matches R's `dgamma(x, shape, scale=...)`. However, many R users default to the `rate` parameterisation (`dgamma(x, shape, rate=...)`), where `rate = 1/scale`. So `Gamma(2, 1)` in Julia is `dgamma(x, 2, rate=1)` or equivalently `dgamma(x, 2, scale=1)` in R — the same here, but be careful when `scale ≠ 1`.
:::
...
::: {.callout-note collapse="true"}
## Coming from R?
R vectorises most functions implicitly — `dpois(obs, lambda, log = TRUE)` works element-wise when `obs` and `lambda` are vectors. In Julia, you must explicitly request element-wise operations by adding `.` (called *broadcasting*). So `logpdf.(dists, obs)` applies `logpdf` to each pair of elements. Similarly, `Poisson.(I)` creates a Poisson distribution for each element of `I`. Forgetting the `.` on a vector operation is a common gotcha for R users.
:::
...
::: {.callout-note collapse="true"}
## Coming from R?
`1.5:0.2:4.0` in Julia is equivalent to `seq(1.5, 4.0, by = 0.2)` in R. The syntax is `start:step:stop`. Note that R's `:` operator (e.g. `1:10`) only increments by 1, whereas Julia's `:` supports any step size.
:::
