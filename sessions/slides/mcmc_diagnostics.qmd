...
## Two more plots worth making {.smaller}

**Running mean** — the posterior mean recomputed as the chain grows.

. . .

It should settle and stay settled. Still drifting after thousands of iterations
means the chain has not forgotten where it started, or has not finished
exploring.
...
It should decay towards zero within a few tens of lags. Still high at lag 100
means successive samples carry almost no new information, and your effective
sample size is a small fraction of the iterations you ran.
...
MCSE is the noise the sampler itself puts on the number you quote.
If you report $R_0 = 2.31$ and the MCSE is $0.05$, that second decimal is sampler
noise: run again with a different seed and it changes.
Sample until the MCSE is small next to the precision you want to claim, whatever
