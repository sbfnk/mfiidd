...
For a model we can **differentiate** — including an ODE model, since the solver
propagates derivatives — the **gradient** of the log-posterior tells us which way
is uphill.

. . .

[Hamiltonian Monte Carlo]{.alert} uses it: instead of a random step, simulate a
puck rolling on the surface. Gradients pull it towards high density, momentum
carries it a long way. Both the direction and the length of each move now come
from the posterior rather than from a proposal we guessed.
...
It usually just works, with no proposal tuning at all, and it is the default to
reach for. Fall back to RAM or MH when the model has discrete parameters, or a
stochastic simulator that breaks differentiation — as the particle filter will
later in the course.
