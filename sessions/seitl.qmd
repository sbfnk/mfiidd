...
function simulate_seitl_stochastic(θ, init_state, times)
    ...
    # Aggregate incidence by day
    # Use interpolation to get cumulative incidence at each time point
    Inc_cumulative_at_times = [sol(t)[6] for t in times]

    # Convert cumulative to daily incidence
    Inc_daily = [0.0; diff(Inc_cumulative_at_times)]

    # Get other state variables at the requested times
    state_matrix = reduce(hcat, [sol(t)[1:5] for t in times])'
    df = DataFrame(state_matrix, [:S, :E, :I, :T, :L])
    df[!, :time] = times
    df[!, :Inc] = Inc_daily
...
## Code explainer
- [`ConstantRateJump(rate, affect!)`](https://docs.sciml.ai/JumpProcesses/stable/jump_types/#Defining-a-Constant-Rate-Jump) - defines a transition event where `rate` is a function returning the event rate, and `affect!` modifies the state when the event occurs.
- [`DiscreteProblem`](https://docs.sciml.ai/JumpProcesses/stable/jump_types/#Defining-Problems-with-Jumps) - defines a problem with discrete states (counts), suitable for jump processes.
- [`JumpProblem(prob, Direct(), jumps...)`](https://docs.sciml.ai/JumpProcesses/stable/jump_types/#JumpProblem) - combines the discrete problem with jump definitions. `Direct()` specifies the Gillespie Direct algorithm for selecting the next jump.
- [`SSAStepper()`](https://docs.sciml.ai/JumpProcesses/stable/algorithms/#JumpProcesses.SSAStepper) - the Stochastic Simulation Algorithm (SSA) stepper, implementing the exact Gillespie algorithm.
:::
...
In the [next session on Particle MCMC](pmcmc.qmd), we'll fit both models to data and formally compare them using cross-validation.
:::
