...
Now, take 10 minutes to explore the dynamics of the model for different parameter and initial state values. In particular, try different values for $R_0\in[2-15]$ and $\alpha\in[0.3-1]$.
...
For stochastic simulation, we implement the Gillespie algorithm (stochastic simulation algorithm) directly. This simulates the model day-by-day, which is natural for fitting to daily incidence data and prepares us for particle filtering later.
...
## Code explainer
- **Gillespie algorithm**: At each step, compute all transition rates, sample the waiting time from an exponential distribution, select which event occurs (weighted by rates), and apply the state change.
- `randexp() / total_rate` - samples the waiting time to the next event. The `randexp()` function draws from Exp(1), and dividing by the total rate gives Exp(total_rate).
- **Stoichiometry matrix** - each row defines how a transition changes the state vector [S, E, I, T, L].
- **Daily incidence** - we track E→I transitions directly (event 2) rather than maintaining a cumulative counter. This is more natural for step-by-step simulation.
:::
...
The same `@model` definitions will be used for particle MCMC inference in the next session, demonstrating the unified approach.
:::

Take about 10 minutes to explore the dynamics of the stochastic SEITL model.
...
Take 5 minutes to have a look at the functions above and make sure you understand how the Erlang distribution for the $T$ compartment is coded.
...
In the [next session on Particle MCMC](pmcmc.qmd), we'll fit both models to data and compare them.
:::
