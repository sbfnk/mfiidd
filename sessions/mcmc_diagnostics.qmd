...
::: {.callout-note collapse="true"}
## Watching divergences appear live (for slow models)

The check above counts divergences *after* the fit finishes, which is fine for the seconds-long fits in this course.
For a slow model you can instead stream the divergence count and log-density out of the sampler as it runs, using a per-iteration `callback`, and stop a doomed fit early.
See [Watching a fit unfold](../reference/live_monitoring.qmd) for the pattern, including an optional TensorBoard dashboard.
:::
