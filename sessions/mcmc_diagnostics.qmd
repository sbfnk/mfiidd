...
Trace plots show parameter values over iterations. They should look like [hairy caterpillars](figures/hairy_caterpillar.jpg):
...
```{julia}
#| output: false
# Run multiple chains for R̂ calculation (sequentially for reproducibility)
# Try with explicit initialization to see if that helps
init_params = [Dict(:R_0 => 2.0, :D_inf => 2.0) for _ in 1:4]
chain_multi = sample(
    model, NUTS(), MCMCSerial(), 2000, 4; init_params = init_params
)
```
...
The model could be mis-specified, missing important features, or based on incorrect assumptions. Posterior predictive checks reveal whether your model actually generates data that looks like what you observed.
