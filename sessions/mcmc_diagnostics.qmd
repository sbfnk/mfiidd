...
# Calculate R̂
println("\nR̂ values:")
summary_values = DataFrame(summarystats(chain_multi))
for row in eachrow(summary_values)
    param = row.param
    value = row.rhat
...
::: {.callout-tip collapse="true"}
## Code explainer
- `chain_multi[chain = i]` - extracts the i-th chain from a multi-chain object. A multi-chain `FlexiChain` is indexed by iteration and chain.
- `summarystats(chain)` - produces a summary table of diagnostics. `DataFrame(summarystats(chain))` has a `:rhat` column holding the R̂ (Gelman-Rubin) diagnostic, which compares variance within chains to variance between chains. Values < 1.1 indicate convergence; < 1.01 is even better.
:::
...
```{julia}
ess_values = DataFrame(summarystats(chain_multi))
n_samples = size(chain_multi, 1)
```

::: {.callout-tip collapse="true"}
## Code explainer
- `summarystats(chain)` - its `DataFrame` has an `:ess_bulk` column holding the Effective Sample Size. MCMC samples are correlated, so N samples don't give you N independent draws. ESS estimates how many independent samples your chain is equivalent to.
:::
...
```{julia}
mcse_values = DataFrame(summarystats(chain_multi))

println("Posterior means ± MCSE:")
for row in eachrow(mcse_values)
    param = row.param
    mcse_val = row.mcse
    # Mean for this parameter is in the same summary row
    mean_val = row.mean
...
println("Divergent transitions: $n_divergent / $(size(chain_good, 1))")
if n_divergent > 0
...
for i in 1:50
    ...
    pred_obs = [predictions[@varname(obs[j])][i, 1] for j in 1:length(epi1.obs)]
...
3. **The result** is stored with keys like `@varname(obs[1])`, `@varname(obs[2])`, etc.
   - Because our model uses `obs ~ arraydist(Poisson.(lambdas))`, Turing names each element of the predicted vector `obs[1]`, `obs[2]`, etc. To access these in Julia, we build the variable name with `@varname(obs[j])`, where the runtime value of `j` is interpolated.
   - `predictions[@varname(obs[j])][i, 1]` then extracts the `i`-th posterior sample for the `j`-th observation. The `1` is the chain index (we have one chain here).
...
```{julia}
# This is what summarystats() reports - split chains for better diagnostics
rhat_split = DataFrame(summarystats(chain_multi))
println("Split-R̂ values (recommended):")
for row in eachrow(rhat_split)
    println("  $(row.param): $(round(row.rhat, digits = 4))")
end
```
...
```{julia}
# Different types of ESS, both columns of the summary table
ess_summary = DataFrame(summarystats(chain_multi))

println("Bulk ESS (central posterior):")
for row in eachrow(ess_summary)
    println("  $(row.param): $(round(row.ess_bulk, digits = 0))")
end

println("\nTail ESS (posterior tails):")
for row in eachrow(ess_summary)
    println("  $(row.param): $(round(row.ess_tail, digits = 0))")
end
```
...
```{julia}
# Diagnose it
println("R̂ and ESS values:")
display(DataFrame(summarystats(chain_problematic)))
```
