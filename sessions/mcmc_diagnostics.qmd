...
**Estimated time**: 2-2.5 hours (multi-chain sampling can be slow)
:::
...
# Concept map: Bayesian inference so far

::: {.callout-note}
## How the pieces fit together

After three sessions, you now have a complete workflow for Bayesian inference with deterministic models:

```
┌─────────────────────────────────────────────────────────────────┐
│                    SESSION 1: INTRODUCTION                      │
│                                                                 │
│   Prior (beliefs)  +  Likelihood (data)  =  Posterior (update)  │
│   ──────────────      ─────────────────      ────────────────   │
│   What you think      How well parameters    Your updated       │
│   before seeing       explain the data       beliefs            │
│   data                                                          │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
...
│                       SESSION 2: MCMC                           │
│                                                                 │
│   Grid search → too slow for real problems                      │
│   MCMC → smart random walks through parameter space             │
│                                                                 │
│   Metropolis-Hastings: simple, needs tuning                     │
│   RAM: adaptive, learns correlations                            │
│   NUTS: gradient-based, automatic, efficient                    │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
...
│                                                                 │
│   Did the algorithm work? Check:                                │
│   • Trace plots    → visual check for mixing                    │
│   • R̂              → chains agree? (< 1.1)                      │
│   • ESS            → enough independent samples? (> 100)        │
│   • Posterior pred → does model match data?                     │
└─────────────────────────────────────────────────────────────────┘
                                │
                                ▼
┌─────────────────────────────────────────────────────────────────┐
│                  NEXT: MODEL CHECKING                           │
│                                                                 │
│   MCMC converged ≠ Model is correct                             │
│   Need to check: does the model describe the data well?         │
└─────────────────────────────────────────────────────────────────┘
```
