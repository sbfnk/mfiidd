...
Why should this ever be faster? Optimisation can use parallel gradient computations and converges in a fixed number of steps, whereas MCMC must run sequentially until convergence. Whether that pays off depends on the model, and on the small one in this session it does not (which can take many thousands of iterations).
