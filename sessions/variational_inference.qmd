...
# `rand(q, n)` returns numbers on that scale: an R_0 near 0 rather than near 10. Each
...
parameters at all: an `R_0` of about 0.07 for a quantity that cannot go below 1, and a `ρ` of about -4 for one bounded in (0, 1).
:::
...
Mean-field assumes parameters are independent. But in epidemic models, $R_0$ and $D_{\text{inf}}$ are **correlated** in the posterior, and here that correlation is positive. It is modest, around 0.27, because the posterior for $D_{\text{inf}}$ is pressed against the upper bound of its prior, which clips the ridge.
