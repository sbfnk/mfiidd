...
The mean-field run above took about as long as MCMC did, which is what a model this small buys you.
...
Mean-field reports the $R_0$ and $D_{\text{inf}}$ marginals about three times too narrow, and leaves $\rho$ alone, where it comes out slightly wider than MCMC.
The narrowing falls on exactly the pair whose correlation it discarded, which is what the independence assumption costs here; full-rank errs slightly wide on all three.
Those narrow marginals are what make mean-field's credible intervals untrustworthy in practice.
