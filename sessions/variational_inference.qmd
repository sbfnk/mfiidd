...
## Code explainer
- [`@elapsed`](https://docs.julialang.org/en/v1/base/base/#Base.@elapsed) - Julia macro that returns the time in seconds to execute the expression. We use this to compare MCMC vs VI speed.
- `NUTS(0.65)` - the argument sets the target acceptance rate during warmup. Values closer to 1 mean smaller, more cautious steps. 0.65 is a good default for complex posteriors; use 0.8-0.9 for difficult geometries.
- `progress=false` - suppresses the progress bar during sampling. Set to `true` if you want to watch progress.
:::
...
It turns out that the speedup becomes even more dramatic with larger models or more samples.
...
**Variational inference** reframes Bayesian inference as optimisation: instead of sampling from the posterior, we find a simple distribution $q(\theta)$ that's as close as possible to the true posterior. This is much faster than MCMC, with speedups that scale with problem complexity.

We explored two approximation families. **Mean-field VI** assumes parameters are independent - very fast, but it missed the strong R₀-D_inf correlation in our SIR model, giving overconfident uncertainty estimates. **Full-rank VI** uses a full covariance matrix, capturing correlations at modest additional cost. For epidemic models with their typical parameter correlations, full-rank is usually worth the overhead.

The practical workflow is: use VI for rapid exploration and real-time applications where MCMC is too slow, but validate against MCMC when accuracy matters. VI is not a replacement for MCMC - it's a complementary tool that enables applications (real-time forecasting, model selection over many candidates) where MCMC would be computationally infeasible.
