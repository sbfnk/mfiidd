...
parameters at all: an `R_0` of about -3.2 for a quantity that cannot go below 1, and a `ρ` of about +4.8 for one that cannot go above it.
:::
...
Mean-field assumes parameters are independent. But in epidemic models, $R_0$ and $D_{\text{inf}}$ are strongly **correlated** in the posterior, and here that correlation is positive: the cell below prints it at about 0.92.
...
The correlation is not the only casualty, though. Mean-field also reports marginals about two and a half times too narrow, while full-rank matches MCMC on those too, and it is the narrow marginals that make its credible intervals untrustworthy in practice.
