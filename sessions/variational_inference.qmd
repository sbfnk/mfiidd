...
R itself does not have a native VI implementation, but if you've used Stan from R, the closest equivalent is **`rstan::vb()`** or **`cmdstanr`'s `$variational()`** method, both of which implement Automatic Differentiation Variational Inference (ADVI). The interface is similar: define a Stan model and call `vb(model)` instead of `sampling(model)`. The same trade-offs apply — faster than full MCMC but approximate. If you have used either of these, the concepts in this session will be familiar.
:::
...
::: {.callout-tip collapse="true"}
## Code explainer
- [`q_meanfield_gaussian(model)`](https://turinglang.org/TuringOptimExt.jl/dev/api_reference/) - creates an initial mean-field Gaussian variational distribution for the model, with independent Normal distributions for each parameter (all initialised to standard normals in unconstrained space).
