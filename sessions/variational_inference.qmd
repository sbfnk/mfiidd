...
The closest equivalent to Turing's VI in the R ecosystem is **rstan's `vb()` function**, which implements Automatic Differentiation Variational Inference (ADVI). The interface is similar: you define a Stan model and call `vb(model)` instead of `sampling(model)`. The same trade-offs apply — faster than full MCMC but approximate. If you have used `rstan::vb()` or `cmdstanr`'s `$variational()` method, the concepts in this session will be familiar.
:::
