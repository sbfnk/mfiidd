...
Each timing cell runs the method once before measuring it, so what you see is the cost of sampling rather than the cost of Julia compiling the sampler for the first time, which on a model this small would otherwise dominate everything.
...
vi(model, q_meanfield_gaussian, 10; adtype=AutoForwardDiff(), show_progress=false)  ## compile first
...
Expect the two timings below to come out close, with full-rank a shade slower.

```{julia}
println("\nRunning Full-Rank VI...")
vi(model, q_fullrank_gaussian, 10; adtype=AutoForwardDiff(), show_progress=false)  ## compile first
...
Once compilation is out of the way, there is almost nothing between the three on this model: all of them finish in around a second.
Variational inference earns its keep on problems where MCMC takes hours, or where the same model has to be refitted to hundreds of datasets, and a three-parameter SIR is neither.
