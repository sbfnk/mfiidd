...
For a single dataset with a simple model, MCMC is usually affordable: the model in this session takes tens of seconds. The problem is what happens next:
...
Note the time printed above, and compare it with the variational runs below rather than with any particular number quoted here: it depends on the machine, and on this model it is tens of seconds rather than the hours a larger model would take.
...
# `rand(q, n)` returns numbers on that scale: an R_0 of -3 rather than of 5. Each
...
parameters at all, with an `R_0` around -3 for a quantity that cannot go below 1.
:::
...
Mean-field assumes parameters are independent. But in epidemic models, $R_0$ and $D_{\text{inf}}$ are strongly **correlated** in the posterior, and here that correlation is positive.
...
With three parameters that cost is invisible, and the timing cell below has full-rank finishing *faster* than mean-field on this run, because the number of optimiser iterations rather than the size of the covariance is what dominates here.
...
The full-rank approximation recovers a correlation close to the one MCMC found, where mean-field was pinned at zero by construction. That is the whole of the difference between the two families on this model.
...
Note how modest the speedup is here: a few fold, on a model with three parameters that MCMC handles comfortably in seconds.
Variational inference earns its keep on problems where MCMC takes hours, or where the same model has to be refitted to hundreds of datasets, and this example is neither.
Comparing the two on a small model is the right way to check that VI is telling the truth, and the wrong way to decide whether you need it.
...
On this three-parameter model the difference is a few fold rather than orders of
magnitude, as the timings above show. Read the table as the shape of the trade-off
rather than as a promise about seconds.
...
for (i, pname) in enumerate(param_names)
    ...
    println("$(rpad(pname, 9)) | $(round(mean(mcmc_vals), digits=2)) ($(round(std(mcmc_vals), digits=2))) | " *
            "$(round(mean(samp_mf[!, pname]), digits=2)) ($(round(std(samp_mf[!, pname]), digits=2))) | " *
            "$(round(mean(samp_full[!, pname]), digits=2)) ($(round(std(samp_full[!, pname]), digits=2)))")
end
```
...
The Gaussian VI methods covered in this session are the most commonly used in practice, but more flexible approaches exist. **Normalising flows** use sequences of invertible transformations to create non-Gaussian approximations, allowing them to handle multimodal and skewed posteriors — see [Bijectors.jl](https://turinglang.org/Bijectors.jl/stable/) for Julia implementations. **Amortised inference** trains a neural network to predict posterior parameters directly from data, eliminating per-dataset optimisation — useful when analysing many similar datasets. These are active research areas; the references below provide entry points.
