...
The generations get *more* expensive, not less: the first is cheap because a loose threshold accepts almost anything, and each tightening costs more simulations per particle than the last. Per particle, this run of ABC-SMC is no cheaper than the rejection sampler was at its looser threshold. What it bought is the tighter threshold itself, which rejection sampling could only reach by rejecting far more.

Watch the effective sample size as well. The first generation has all 100 particles, because they are drawn from the prior and weighted equally, and after that it collapses: in this run to single figures in generation 2, recovering only partially in generation 3. The exact numbers move from run to run, but the collapse does not.
