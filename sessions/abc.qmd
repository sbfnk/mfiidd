...
::: {.callout-tip title="Solution" collapse="true"}
L2 reacts most strongly. It squares the deviation, so a single day off by 30
contributes 900 while thirty days off by 1 contribute 30 between them. Squared
distances are dominated by their worst point.
...
::: {.callout-tip title="Solution" collapse="true"}
L2 reacts most strongly: it squares each discrepancy, so a single large deviation
dominates the total. L1 grows only linearly with the discrepancy, which makes it
the most outlier-tolerant of the three. The weighted distance sits in between,
and where exactly depends on whether the outlier falls at a time point the
weighting emphasises.

This is the same trade-off as choosing between a Poisson and a negative binomial
likelihood — which is the point of the section that follows.
:::
