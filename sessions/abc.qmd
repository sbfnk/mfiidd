...
# A well-fitting simulation, for contrast: roughly the posterior mean from the
# likelihood fit above
good_sim = simulate_sir_abc([1.76, 3.27, 0.99])
...
the previous one. Returns the particles, their importance weights, and the number
of simulations each generation cost.
"""
function abc_smc(prior_sampler, simulator, distance_fn, observed_data,
                thresholds, n_particles; max_tries_per_particle = 20_000)
    ...
    for (t_idx, threshold) in enumerate(thresholds)
        ...
        tries = zeros(Int, n_particles)

        Threads.@threads for i in 1:n_particles
            for attempt in 1:max_tries_per_particle
                θ = if t_idx == 1
                    prior_sampler()
                else
                ...
                attempt == max_tries_per_particle &&
                    error("no particle accepted at threshold $threshold after " *
                          "$max_tries_per_particle simulations; loosen the schedule")
            end
        end
        ...
        weights = new_weights
        push!(sim_counts, sum(tries))

        weighted_R_0 = sum(weights[i] * particles[i][1] for i in eachindex(particles))
        println("  Simulations: $(sum(tries)),  weighted mean R_0: ",
                round(weighted_R_0, digits=2))
    end

    return particles, weights, sim_counts
end
```
:::
...
abc_smc_samples, abc_smc_weights, abc_smc_sims = abc_smc(
    sample_from_prior,
    simulate_sir_abc,
    distance_L1,
    observed_data,
...
Compare the simulation counts printed above rather than taking the efficiency claim on trust.
ABC-SMC reaches a tighter threshold, 250 against 300, and the generation-by-generation counts show where its budget goes: the first generation is as expensive as rejection sampling, because it *is* rejection sampling, and the later ones are cheap because they start from particles that already fit.
That is the whole trick, and it is why the advantage grows in higher dimensions, where a prior draw almost never lands anywhere useful.
...
- ABC-SMC reaches a given threshold for far fewer simulations than rejection
  sampling, by walking the threshold down in stages
