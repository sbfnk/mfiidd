...
```{julia}
# The prior, written twice: once to sample from and once to evaluate. ABC-SMC
# needs both, because the importance weights below are a ratio of prior density
# to perturbation density.
...
function abc_smc(prior_sampler, in_prior, simulator, distance_fn, observed_data,
    ...
    for (t_idx, threshold) in enumerate(thresholds)
        ...
        # previous generation was to propose it. Skipping this step, which is
        # tempting because the arithmetic is fiddly, leaves the sample biased
        # towards wherever the kernel happens to concentrate.
...
- ABC-SMC walks the threshold down in stages, which reaches thresholds rejection
  sampling cannot afford, at the cost of weights that grow steadily more uneven
