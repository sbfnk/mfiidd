...
chain_poisson = sample(model_poisson, NUTS(0.65), 1000, progress=true)
```
...
# Negative Binomial with different φ
negbin_distance_phi5 = [-logpdf(NegativeBinomial(λ, 5.0), obs_val) for λ in predicted_range]
negbin_distance_phi20 = [-logpdf(NegativeBinomial(λ, 20.0), obs_val) for λ in predicted_range]
...
threshold = 200.0  # Adjust based on data scale
...
**Key insight**: Choosing a distance metric in ABC is philosophically similar to choosing a parametric likelihood!
...
function abc_smc(prior_sampler, simulator, distance_fn, observed_data,
    ...
    for (t_idx, threshold) in enumerate(thresholds)
        ...
        if t_idx == 1
            # First generation: sample from prior
            for i in 1:n_particles
                while true
                    ...
                    if d < threshold
                        push!(new_particles, θ)
                        break
                    end
                end
            end
        else
            # Later generations: perturb from previous
            for i in 1:n_particles
                while true
...
thresholds = [300.0, 200.0, 150.0, 100.0]  # Adaptive sequence
...
1. **Implement Gaussian likelihood**: Add a `sir_gaussian` model with Normal observation errors. Compare posteriors to Poisson and NegBin.

2. **Custom ABC distance**: Define a distance function that only uses (a) total cases and (b) peak timing. How do posteriors change?

3. **Overdispersion detector**: Compute variance/mean ratio for TdC data. Does this justify using NegBin over Poisson?

4. **Outlier robustness**: Add artificial outliers to the data. Compare posterior estimates under Poisson (sensitive) vs ABC with robust distance metric.

5. **Summary statistics**: Instead of using all daily counts, use summary statistics: total cases, peak time, peak height. Implement ABC with these summaries.
...
- Cranmer, K. et al. (2020). The frontier of simulation-based inference. *PNAS* 117(48), 30055-30062.
