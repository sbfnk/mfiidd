...
prior_samples = sample(sir_model(times, S0, I0, 60), Prior(), 200, progress=false)
```
...
::: {.callout-tip title="What to look for" collapse="true"}
Peak size and total cases usually sit comfortably inside the posterior predictive
distribution: the model has two free parameters and can scale the epidemic to
match how many people were infected.

Peak timing is the one that tends to fall in the tail. That is the signature of a
model whose *shape* is wrong rather than whose scale is wrong — it can be made to
reach the right height, but not at the right moment, because a single SIR wave
cannot bend the way the data do.
