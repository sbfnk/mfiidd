...
ppc_samples = [
    [predictions[@varname(obs[j])][i, 1] for j in 1:n_obs]
    for i in 1:n_sims
]
```
...
The result is a chain-like object where `predictions[@varname(obs[j])]` gives the predicted values for the `j`-th observation across all posterior samples.
:::
