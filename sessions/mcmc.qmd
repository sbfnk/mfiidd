...
::: {.callout-note collapse="true"}
## Coming from R?
Turing's `@model` with `~` syntax is similar to model definitions in NIMBLE, BUGS/JAGS, and Stan:
...
```r
# rstan
fit <- sampling(stan_model, data = list(...), iter = 2000)
summary(fit)
as.array(fit, pars = "x")

# cmdstanr
fit <- stan_model$sample(data = list(...), iter_sampling = 2000)
fit$summary()
fit$draws("x")
```

```julia
# Turing.jl equivalents
