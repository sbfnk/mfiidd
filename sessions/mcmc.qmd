...
::: {.callout-note collapse="true"}
## Coming from R?
If you've used probabilistic programming from R, you'll recognise the `~` syntax. Turing's `@model` is similar to model definitions in NIMBLE, BUGS/JAGS, and Stan (which R users typically access via `rstan` or `cmdstanr`):
...
::: {.callout-note collapse="true"}
## Coming from R?
If you've used Stan from R, the workflow is similar: `sample()` runs inference and returns a chain object, `describe()` gives summary statistics, and `chain[:x]` extracts samples for a parameter.
