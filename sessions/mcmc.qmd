...
You'll build understanding by coding a simple MCMC algorithm yourself, then see how modern tools like [Turing.jl](https://turinglang.org/stable/) automate everything with state-of-the-art methods.
...
## SIR Model Definition (from [`src/sir_model.jl`](https://github.com/sbfnk/mfiidd/blob/main/src/sir_model.jl))
...
Because our hand-written sampler has the chain state in hand at every step, we can also *watch* it explore by animating the trace as it fills in.
...
This is the same "watch good vs bad behaviour as it happens" intuition the post-hoc diagnostics in the [next session](mcmc_diagnostics.qmd) quantify with R̂ and effective sample size.

::: {.callout-note collapse="true"}
## Watching a Turing fit unfold

Turing's samplers expose the same idea through a per-iteration `callback`, so you can stream live diagnostics out of `sample(...)` — useful for slow models where you want to spot trouble before the run finishes.
See [Watching a fit unfold](../reference/live_monitoring.qmd) for the pattern, including a TensorBoard dashboard.
:::
...
Turing's `@model` macro is convenient syntax, but it's not the only way to do MCMC in Julia. In the [Introduction](introduction.qmd) you built a log-posterior function by hand — that same function can be plugged into standalone samplers like [DynamicHMC.jl](https://github.com/tpapp/DynamicHMC.jl) via the [LogDensityProblems.jl](https://github.com/tpapp/LogDensityProblems.jl) interface. This gives you access to samplers beyond what Turing offers, with fewer dependencies. See the [reference page](../reference/logdensity_interface.qmd) for a worked example.
:::
...
::: {.callout-tip title="Hint 1: Interpreting acceptance rates (click if stuck after 2-3 attempts)" collapse="true"}
...
::: {.callout-tip title="Hint 2: Why do R₀ and D_inf need different scales? (click if one parameter mixes well but not the other)" collapse="true"}
Look at the posterior uncertainty for each parameter:
- R₀ might range from ~1.5 to ~3.5 (spread of ~2)
- D_inf might range from ~1.5 to ~3.0 (spread of ~1.5)
...
::: {.callout-tip title="Hint 3: Why might correlation help? (click if diagonal proposals aren't working)" collapse="true"}
