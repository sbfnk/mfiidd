...
using SciMLSensitivity  # adjoints for reverse-mode AD through the ODE solve
...
# Define network structure - small and fast to train
...
# Set up optimisation with reverse-mode AD (Zygote) for gradient computation
opt_func = OptimizationFunction(loss_ude, Optimization.AutoZygote())
...
- `Optimization.AutoZygote()` - uses [Zygote.jl](https://fluxml.ai/Zygote.jl/) for reverse-mode automatic differentiation, with [SciMLSensitivity.jl](https://docs.sciml.ai/SciMLSensitivity/stable/) providing the adjoint that propagates gradients back through the ODE solve. Reverse-mode is the usual choice for training neural networks, where one scalar loss depends on many parameters.
...
Automatic differentiation computes exact gradients of the loss with respect to parameters. We use reverse-mode AD ([Zygote.jl](https://fluxml.ai/Zygote.jl/), with [SciMLSensitivity.jl](https://docs.sciml.ai/SciMLSensitivity/stable/) handling the gradient through the ODE solve), the standard choice for training neural networks.
...
for seed in [1234, 5678, 9012]
    ...
    opt_func_test = OptimizationFunction(loss_ude, Optimization.AutoZygote())
