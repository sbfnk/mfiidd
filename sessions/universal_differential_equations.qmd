...
## cases fell ill during the voyage rather than on the island. Dropping the
## leading point then lines the incidence up with the days the observations count
...
incidence_constant = -diff(S_values)  ## drop the leading day, to match the observations
...
function predict_ude(p_all)
    ...
    return -diff(S_pred)  ## drop the leading day, to match the observations
end
...
to about 3 as the first wave takes off, falls to about 0.3 by day 15, and is then
held a little above 1 through days 20 to 30, which is what sustains the second wave.
