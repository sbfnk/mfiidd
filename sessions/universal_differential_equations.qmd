...
that window it produces about 91 cases against the 104 observed. A time-varying β
...
days 20 and 31 the susceptible fraction falls from about 35% to 21%, and the
...
Past day 31 it stops keeping up. R₀ goes on climbing, but not fast enough, and
R_eff drifts down to about 0.65 by day 59 as the tail decays.
...
The fit itself is not the problem here: time-varying transmission does reproduce both waves. What it costs is the problem. By day 30, where the data still constrain the answer, the implied R_0 is already about 5, twice the literature value the constant-β baseline uses, and it is still rising. That is a strong hint that a different structure would explain the data more plausibly.
