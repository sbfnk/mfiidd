...
```
β(t) = NeuralNetwork([I/N, t])  # Must be positive
```
...
Ours drops steeply over the first ten iterations, rebounds, then descends slowly and unevenly before flattening out. Running to 10,000 iterations gets the loss to within about two units of where 40,000 leaves it, so this is close enough to converged to interpret.
...
The constant-β baseline nearly matches the height of the first peak, about 41
against an observed 47, and then collapses to nothing by day 15. It has one wave
to give and it spends it on the first.

The UDE reaches a similar peak, around 42, and then keeps generating cases through
days 20 to 50, where the data really do have them and the baseline has none. Over
that window it produces about 89 cases against the 104 observed. A time-varying β
is what lets one model do both, which is the whole claim of the approach and the
...
roughly 2.8 as the first wave takes off, falls to about 0.3 by day 15 as that wave
burns through the susceptibles, and is then held close to 1 through days 20 to 30,
which is what sustains the second wave. After that it settles a little below 1 and
the tail decays.

Now read R₀ against it. To keep R_eff near 1 while the susceptible fraction falls
from about 35% to under 5%, the implied R₀ has to climb from about 3 to nearly 19.
The network is not describing a change in behaviour; it is buying back, through
transmissibility, exactly what susceptible depletion takes away.
...
Be careful about what that shape can be attributed to. The network takes
prevalence and time, and along the fitted trajectory β is at a low exactly when
prevalence peaks, around day 10, then climbs through the second wave. Read one
way that is a behavioural response: transmission falls while the epidemic is
visibly large. Read another way it is susceptible depletion, since S is *not* an
input and falls monotonically with time, so anything the network learns about the
shrinking susceptible pool has to be expressed through the time input. Those two
readings are not separable from this fit.

The reason is worth holding on to, because it is a property of the method rather
than of this dataset. The training data trace a single path through the network's
two-dimensional input space: each day corresponds to one prevalence, so the
network is only ever constrained along that curve. Ask it what β would be at a
prevalence the epidemic never reached on a given day and it will answer, but
nothing in the data informed that answer.
...
In the section on [structural constraints](#the-deeper-issue-structural-constraints), we argue that an SIR model with learned β(t) can only sustain the second wave by driving β ever higher. Can you explain, in your own words, why even a very large β during the trough is insufficient — considering both the depleted susceptible pool and near-zero prevalence? What structural change to the model would you propose, and how does this relate to the SEIT4L model from the [pMCMC session](pmcmc.qmd)?
:::
