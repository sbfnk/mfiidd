...
**What to expect**: the loss should fall steadily and substantially. Do not expect it to fall by orders of magnitude here, and do not use that as your check: a Poisson negative log-likelihood is not bounded below by zero, and ours goes from about +356 to about -460, so ratios of it mean nothing. Watch the shape instead. A loss that sits exactly flat means no gradient is reaching the parameters, which is a bug rather than a converged fit; `NaN` usually means the learning rate is too high; and a curve still falling steeply at the end means run for longer.
...
a little over 3 as the first wave takes off, falls to about 0.3 by day 15 as that wave
...
about 1.9 over the first six days while prevalence is still rising, turns down
before prevalence peaks on day 8, and reaches its lowest point on day 14 once the
wave has largely passed. Transmission rising as the epidemic grows and falling as
...
The fit itself is not the problem here: time-varying transmission does reproduce both waves. What it costs is the problem, and an R_0 near 19 is a strong hint that a different structure would explain the data more plausibly.
