...
β_guess = 0.5
γ_guess = 0.4  # mean infectious period ≈ 2.5 days, consistent with influenza
u0 = [270.0, 3.0, 0.0]
...
What transmission pattern did the network learn?
...
An imperfect fit is informative: it suggests that time-varying transmission alone is insufficient to explain the data, and that we may need different model structure.
...
```{julia}
# Train from 3 different random seeds and compare final losses
for seed in [1234, 5678, 9012]
    ...
    result_test = solve(opt_prob_test, Optimisers.Adam(0.01), maxiters=100,
                        callback = (state, l) -> false)
    println("Seed $seed → final loss: $(round(result_test.objective, digits=2))")
end
```

If the final losses are similar across seeds, the ODE constraint is doing its job — the mechanistic structure limits the solution space enough that different starting points converge to similar answers. If they vary substantially, this tells you the data can't uniquely determine β(t), and you should treat any single result with caution.
