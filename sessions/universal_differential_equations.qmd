...
function predict_ude(p_all)
    ...
    T = eltype(p_all)
    u0_promoted = T.(u0)
    prob_ude = ODEProblem(sir_ude!, u0_promoted, tspan, p_all)
    ...
    if !SciMLBase.successful_retcode(sol_ude)
        return fill(T(NaN), length(times))
    end

    S_pred = sol_ude[1, :]
    incidence_pred = [zero(T); -diff(S_pred)]
...
Ours drops steeply over the first ten iterations, rebounds, then descends slowly and unevenly for the rest of the run. It is *still falling* at iteration 500, by about 0.09 per iteration over the last twenty. By the advice in this section that means the fit has not converged and more iterations are warranted; we stop at 500 because this is a teaching example and the shape of the answer is already visible, which is a trade you should make deliberately rather than by not looking.
...
**What to expect**: the loss should fall steadily and substantially. Do not expect it to fall by orders of magnitude here, and do not use that as your check: a Poisson negative log-likelihood is not bounded below by zero, and ours goes from about +356 to about -340, so ratios of it mean nothing. Watch the shape instead. A loss that sits exactly flat means no gradient is reaching the parameters, which is a bug rather than a converged fit; `NaN` usually means the learning rate is too high; and a curve still falling steeply at the end means run for longer.
...
The answer is not the one you might expect. The constant-β baseline nearly matches
the height of the first peak, about 41 against an observed 47, and then collapses
to nothing by day 15. The UDE does the opposite: it peaks at about 11, missing the
first wave badly, and in exchange keeps a low tail running through days 20 to 50
where the data really do have cases and the baseline has none.

That trade is a direct consequence of the loss we chose, though not for the reason
usually given. Day by day, a Poisson negative log-likelihood still charges more for
a large absolute miss than a small one: missing by thirty on a day expecting forty
costs several times what missing by five on a day expecting two does.

What decides the outcome is arithmetic across the series. Predicting *nothing* on a
day when cases occurred is close to unbounded under a log-likelihood, and the
constant-β baseline predicts essentially nothing for the whole of days 20 to 50.
Adding those forty days up outweighs everything the UDE gives away at the peak, so
the optimiser buys the tail and sells the peak. Swap in sum of squared errors and
the incentive reverses: an empty tail is cheap, a peak missed by thirty is not, and
the fit would go the other way.

So the network did learn something, and it is not what a glance at the peak would
suggest. Neither curve reproduces both waves, which is the point the next section
takes up.
...
The curve above starts at about 1.0, a little *below* the constant-β baseline,
falls to a minimum near 0.7 around day 8, and then climbs steadily for the rest of
the outbreak, crossing the baseline around day 24 and reaching roughly 2.5 by day
59.

Read alongside the fit, that shape says what the network was doing. It suppresses
transmission through the first wave, which is how the trajectory stays down at 11
cases instead of overshooting to 40, and then raises transmission continuously to
keep generating the small numbers of cases in the tail. It is propping up a long
low tail rather than producing a second wave, and it needs an ever-larger β to do
so because the susceptible pool is draining the whole time.
...
relaxation: initial caution, then a return to normal contact patterns.

A β(t) that rises without ever coming back down, as ours does, is a different
signal. It says the network is compensating for something the *structure* cannot
do, rather than describing a behaviour. Here it is compensating for a susceptible
pool that only ever shrinks.
