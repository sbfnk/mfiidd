...
The curve above starts low, well *below* the constant-β baseline, and climbs for
the rest of the outbreak, ending several times the baseline value by day 59.

Read alongside the fit, that shape says what the network was doing. A low β early
is enough to produce the first wave, because almost everyone is still susceptible.
Keeping cases coming later takes an ever-larger β, because the susceptible pool
has been draining the whole time and each infectious person meets fewer people who
can be infected. The rise is the network compensating for that depletion.
...
pool that only ever shrinks, which a single closed SIR compartment forces on it.
Real reintroduction or waning immunity would show up this way too, and the shape
