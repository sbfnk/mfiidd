...
Always check that the loss has converged — if it is still decreasing at the end, run more iterations.
The curve below drops steeply for the first twenty iterations, sits on a plateau near -200 until about iteration 200, then falls again to around -340 and flattens out.
A plateau in the middle of training is worth noticing: had we stopped at 150 iterations we would have concluded, wrongly, that the fit had converged.
...
That trade is a direct consequence of the loss we chose. A Poisson negative
log-likelihood is dominated by relative error, so being wrong by five cases on a
day expecting two costs far more than being wrong by thirty on a day expecting
forty. There are roughly forty small-count days in the tail and about five large
ones at the peak, and the optimiser goes where the cost is. Swap in sum of squared
errors and the incentive reverses: the peak would dominate and the tail would be
abandoned.
