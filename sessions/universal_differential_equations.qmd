...
already in the model: the `S` factor in `-β * S * I / N` accounts for it, which is
exactly what R_eff divides by. So a β that has to climb is not the network
...
What the network's inputs can support is also narrower than it looks. Along the
fitted trajectory β is at a low just as prevalence peaks around day 10, which
reads as a behavioural response to a visibly large epidemic. But the training data
trace a single path through the network's two-dimensional input space: each day
corresponds to one prevalence, so dependence on prevalence and dependence on time
cannot be told apart from this fit. Ask the network what β would be at a
